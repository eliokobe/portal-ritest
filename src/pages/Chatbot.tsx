...
const Chatbot: React.FC = () => {
  ...
  const handleSaveComments = async () => {
    if (!commentingContractId) return;
    setSaving(true);
    try {
      await airtableService.updateContractField(commentingContractId, 'Comentarios', editingComments);
      setContracts(prev => prev.map(c => 
        c.id === commentingContractId ? { ...c, Comentarios: editingComments } : c
      ));
      setShowCommentsModal(false);
      setCommentingContractId(null);
    } catch (error) {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-64">
        ...
        <p className="ml-4 text-gray-600">Cargando contratos...</p>
      </div>
    );
  }
