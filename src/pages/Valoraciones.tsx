...
const Valoraciones: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const loadValoraciones = async () => {
      setLoading(true);
      try {
        console.log('Valoraciones - Starting to load...');
        
        const data = await airtableService.getValoraciones();
        console.log('Valoraciones - Data received:', data.length, 'records');
        console.log('Valoraciones - Datos completos:', data);
        
        if (isMounted) {
          setValoraciones(data);
        }
      } catch (error: any) {
        console.error('Valoraciones - Error fetching data:', error);
        if (isMounted) {
    ...
    const loadCodigos = async () => {
      try {
        ...
        console.log('Códigos sin enviar cargados:', data.length);
        setCodigosDisponibles(data);
      } catch (error) {
        console.error('Error loading códigos:', error);
      }
    };
  ...
  const filteredValoraciones = useMemo(() => {
    // Filtrar primero por estado: excluir "Validado"
    const valoracionesPendientes = valoraciones.filter((v) => {
      const estado = v.estado?.trim() || '';
      const isValidado = estado.toLowerCase() === 'validado';
      console.log('Filtrando valoración:', { id: v.id, estado: v.estado, isValidado });
      return !isValidado;
    });
    
    console.log('Valoraciones después de filtrar:', valoracionesPendientes.length, 'de', valoraciones.length);
  ...
  const handleValoracionChange = async (valoracion: Valoracion, newValue: number) => {
    if (newValue === valoracion.valoracionCliente) return;

    setSaving(true);
    try {
      await airtableService.updateValoracionCliente(valoracion.id, newValue);
      
      const updatedValoraciones = valoraciones.map((v) =>
        v.id === valoracion.id ? { ...v, valoracionCliente: newValue } : v
      );
      setValoraciones(updatedValoraciones);
    } catch (error) {
      console.error('Error updating valoración:', error);
  ...
  const handleCodigosChange = async (valoracion: Valoracion, newCodigosIds: string[]) => {
    ...
    try {
      ...
      console.error('Error updating códigos:', error);
  ...
  const handleOpenCodigosModal = (valoracion: Valoracion) => {
    setSelectedValoracionForCodigos(valoracion);
    setTempSelectedCodigos(valoracion.codigos || []);
  };
  ...
  const renderStars = (valoracion: Valoracion) => {
    const currentValue = valoracion.valoracionCliente || 0;
    ...
    return (
      <div className="flex gap-1">
        {[1, 2, 3, 4, 5].map((star) => (
          <button
            key={star}
            onClick={() => handleValoracionChange(valoracion, star)}
            disabled={saving}
            className={`transition-colors ${saving ? 'cursor-not-allowed opacity-50' : 'cursor-pointer hover:scale-110'}`}
          >
            <Star
              className={`h-5 w-5 ${
                star <= currentValue
