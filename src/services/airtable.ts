...
export const airtableService = {
  ...
  async updateRegistro(registroId: string, updates: Partial<Omit<Registro, 'id'>>): Promise<void> {
    try {
      ...
      if (updates.cita !== undefined) fields['Cita'] = updates.cita;
      if (updates.comentarios !== undefined) fields['Comentarios'] = updates.comentarios;
      if (updates.informe !== undefined) fields['Informe'] = updates.informe;
  ...
  async getFormularioByExpediente(expediente: string): Promise<any | null> {
    try {
      const expedienteEsc = String(expediente).replace(/'/g, "\\'");
      const params = {
        filterByFormula: `{Expediente} = '${expedienteEsc}'`,
        maxRecords: 1,
      };
      
      const records = await fetchAllServicios('Formulario', params);
      ...
      if (records.length > 0) {
        ...
        return {
          ...
          problema: f['Problema'],
          detalles: f['Detalles'] ?? f['Details'],
          // Archivos adjuntos (asumiendo que son campos de tipo Attachment)
          archivo1: f['Archivo 1'] ?? f['File 1'],
          archivo2: f['Archivo 2'] ?? f['File 2'],
          archivo3: f['Archivo 3'] ?? f['File 3'],
          // Fotos
          fotoGeneral: f['Foto general'] ?? f['Foto General'],
          fotoEtiqueta: f['Foto etiqueta'] ?? f['Foto Etiqueta'],
          fotoRoto: f['Foto roto'] ?? f['Foto Roto'],
          // Incluir todos los demás campos que pueda tener la tabla
          ...f,
        };
      }
      return null;
  ...
  async getReparacionesByExpediente(expediente: string): Promise<any | null> {
    try {
      const expedienteEsc = String(expediente).replace(/'/g, "\\'");
      const params = {
        filterByFormula: `{Expediente} = '${expedienteEsc}'`,
      ...
      const records = await fetchAllServicios('Reparaciones', params);
      ...
      if (records.length > 0) {
        ...
        return {
          ...
          cuadroElectrico: f['Cuadro eléctrico'] ?? f['Cuadro electrico'],
  ...
  async updateFormularioField(formId: string, field: string, value: any): Promise<void> {
    try {
  ...
  // Actualizar campo de reparaciones
  async updateReparacionField(repId: string, field: string, value: any): Promise<void> {
    try {
  ...
  async getTasks(): Promise<{
    ...
    try {
      ...
      return records.map((r: any) => {
        ...
        return {
          id: r.id,
          tarea: f['Tarea'] ?? f['Task'] ?? '',
          estado: f['Estado'] ?? f['Status'] ?? 'Pendiente',
          prioridad: f['Prioridad'] ?? f['Priority'],
          fechaLimite: f['Fecha límite'] ?? f['Fecha limite'] ?? f['Due Date'],
  ...
  async createTask(task: {
    ...
    try {
      await serviciosApi.post('/Tareas', {
        records: [{
          fields: {
            'Tarea': task.tarea,
            'Estado': task.estado,
  ...
  async updateTaskStatus(taskId: string, estado: string): Promise<void> {
    try {
      await serviciosApi.patch(`/Tareas/${taskId}`, {
        fields: {
          'Estado': estado,
        },
      });
  ...
  async deleteTask(taskId: string): Promise<void> {
    try {
      await serviciosApi.delete(`/Tareas/${taskId}`);
    } catch (error) {
  ...
  async getEnvios(): Promise<{
    ...
    try {
      const records = await fetchAllServicios('Envíos', { pageSize: 100 });
      ...
      return records.map((r: any) => {
        ...
        return {
          ...
          tipoOperacion: f['Tipo de operación'],
          estado: f['Estado'],
          fechaEnvio: f['Fecha de envío'],
          material: Array.isArray(f['Material']) ? f['Material'].join(', ') : f['Material'],
          producto: Array.isArray(f['Producto']) ? f['Producto'].join(', ') : f['Producto'],
          fechaCambio: f['Fecha cambio'] ?? r.createdTime,
        };
      });
    } catch (error) {
  ...
  async createEnvio(envio: Omit<{
    ...
    producto?: string;
    fechaCambio?: string;
  }, 'id' | 'producto' | 'fechaCambio'>): Promise<void> {
    try {
      await serviciosApi.post(`/Envíos`, {
        records: [{
          fields: {
            'Seguimiento': envio.seguimiento || '',
            'Servicio': envio.servicio ? [envio.servicio] : [],
            'Estado': envio.estado || 'Envío creado',
            'Fecha de envío': envio.fechaEnvio || '',
            'Material': envio.material ? [envio.material] : [],
          }
        }]
      });
    } catch (error) {
  ...
  // Actualizar envío
  async updateEnvio(envioId: string, fields: Partial<{
    seguimiento?: string;
    tipoOperacion?: 'Envío' | 'Recogida';
    estado?: 'Envío creado' | 'Paquete recogido' | 'Paquete entregado';
    fechaEnvio?: string;
    ...
    try {
      await serviciosApi.patch(`/Envíos/${envioId}`, {
        fields: {
          'Seguimiento': fields.seguimiento,
          'Estado': fields.estado,
          'Fecha de envío': fields.fechaEnvio,
          'Material': fields.material ? [fields.material] : undefined,
          'Servicio': fields.servicio ? [fields.servicio] : undefined,
        },
      });
    } catch (error) {
